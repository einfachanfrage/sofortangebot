  ...
  return {
    h2: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
      <h2 id={slugify(String(children))} {...props}
        className="font-extrabold text-[#2C2C2C] text-[24px] mt-10 mb-3 pl-4 border-l-4 border-[#F5C400] leading-snug"
      >{children}</h2>
    ),
    h3: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
      <h3 id={slugify(String(children))} {...props}
        className="font-extrabold text-[#2C2C2C] text-[20px] mt-8 mb-2 leading-snug"
      >{children}</h3>
    ),
    ...
    blockquote: ({ children, ...props }: React.HTMLAttributes<HTMLQuoteElement>) => (
      <blockquote {...props}
        className="border-l-4 border-[#F5C400] bg-[#FFFBEB] px-5 py-4 my-6 rounded-r-lg italic text-[#555] text-[16px] leading-relaxed"
      >{children}</blockquote>
    ),
    strong: ({ children, ...props }: React.HTMLAttributes<HTMLElement>) => (
      <strong {...props} className="font-extrabold text-[#2C2C2C]">{children}</strong>
    ),
...
function InlineCTA() {
  return (
    <div className="border-l-4 border-[#F5C400] bg-[#FFFBEB] px-5 py-5 my-8 rounded-r-lg">
      <div className="font-extrabold text-[#2C2C2C] text-[16px] mb-2">
      ...
      <Link
        href="/register"
        className="inline-flex items-center gap-2 bg-[#F5C400] text-[#2C2C2C] font-extrabold text-sm px-5 py-2.5 rounded-lg hover:bg-[#e6b800] transition-colors"
      >
    ...
    <>
      ...
      <div className="min-h-dvh bg-[#F7F7F5]">
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-10 py-8">
          {/* Breadcrumb */}
          <div className="flex items-center gap-1.5 text-[13px] text-[#2C2C2C]/40 font-medium mb-6">
            <Link href="/" className="hover:text-[#2C2C2C]/70">Startseite</Link>
            <span>›</span>
            <Link href="/blog" className="hover:text-[#2C2C2C]/70">Blog</Link>
            <span>›</span>
            <span className="text-[#2C2C2C]/60 truncate max-w-[200px]">{fm.title}</span>
          </div>
          ...
          <div className="flex gap-12 items-start">
            ...
            <article className="flex-1 min-w-0 max-w-[680px]">
              ...
              <div className="flex items-center gap-2.5 mb-4 flex-wrap">
                ...
                <span className="text-[#2C2C2C]/30 text-xs">·</span>
                <span className="text-[#2C2C2C]/40 text-xs font-semibold">{post.lesezeit}</span>
                <span className="text-[#2C2C2C]/30 text-xs">·</span>
                <span className="text-[#2C2C2C]/40 text-xs font-semibold">{formatDate(fm.date)}</span>
              </div>

              <h1 className="font-extrabold text-[#2C2C2C] text-[32px] md:text-[40px] leading-tight tracking-tight mb-8">
                {fm.title}
              </h1>
              ...
              {/* Related posts */}
              {related.length > 0 && (
                <div className="mt-14 pt-10 border-t border-[#2C2C2C]/10">
                  <h2 className="font-extrabold text-[#2C2C2C] text-xl mb-5">Das könnte dich auch interessieren</h2>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {related.map(p => {
                      const rm = KATEGORIE_META[p.kategorie] ?? KATEGORIE_META['Angebote']
                      return (
                        <Link key={p.slug} href={`/blog/${p.slug}`}
                          className="bg-white rounded-lg p-4 border border-[#2C2C2C]/5 hover:shadow-sm hover:-translate-y-0.5 transition-all group"
                        >
                          <span className="text-xs font-bold px-2 py-0.5 rounded-full mb-2 inline-block" style={{ color: rm.color, background: rm.bg }}>
                            {p.kategorie}
                          </span>
                          <div className="font-extrabold text-[#2C2C2C] text-sm leading-snug group-hover:text-[#F5C400] transition-colors">
                            {p.title}
                          </div>
                        </Link>
                      )
