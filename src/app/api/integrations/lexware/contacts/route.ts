...
export async function GET(req: NextRequest) {
  ...
  const { data: company } = await supabase.from('companies').select('lexware_api_key').eq('user_id', user.id).single()
  if (!company?.lexware_api_key) return NextResponse.json({ contacts: [] })

  const res = await fetch(
    `https://api.lexoffice.io/v1/contacts?name=${encodeURIComponent(q)}&page=0&size=10`,
    { headers: { 'Authorization': `Bearer ${company.lexware_api_key}`, 'Accept': 'application/json' } }
  )
  ...
  const contacts = (data.content ?? []).map((c: {
    ...
    return { id: c.id, name, address, email, phone, source: 'lexware' }
