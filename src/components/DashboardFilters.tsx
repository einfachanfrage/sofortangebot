  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative md:max-w-sm">
        <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-anthracite/30" strokeWidth={2.5} />
        <input
          type="search"
          placeholder="Kunde suchen..."
          defaultValue={q}
          onChange={e => update('q', e.target.value)}
          className="w-full bg-white border border-anthracite/10 rounded-xl pl-9 pr-4 py-2.5 text-anthracite font-semibold text-sm focus:outline-none focus:border-yellow"
        />
      </div>
            ...
            <button
              ...
              className={`shrink-0 text-xs font-black px-3 py-1.5 rounded-full transition-colors flex items-center gap-1.5 ${
                isActive
                  ...
                  : 'bg-white border border-anthracite/10 text-anthracite/60'
              }`}
            >
              {pill.label}
