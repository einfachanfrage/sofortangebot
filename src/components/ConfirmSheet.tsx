  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center md:justify-center">
      ...
      <div className="relative w-full md:max-w-sm bg-white rounded-t-3xl md:rounded-3xl px-5 pt-4 pb-10 md:pb-6 shadow-2xl">
        ...
        <h2 className="font-syne font-extrabold text-[#2C2C2C] text-[20px] mb-2">{title}</h2>
        {text && (
          <p className="text-[#2C2C2C]/50 font-semibold text-[14px] mb-6 leading-relaxed">{text}</p>
        )}
        <div className="flex flex-col gap-3">
          <button
            onClick={onConfirm}
            className={`w-full rounded-2xl py-4 font-extrabold text-[16px] active:scale-[0.98] transition-all ${
              destructive ? 'bg-red-500 text-white' : 'bg-[#F5C400] text-[#2C2C2C]'
            }`}
          >
            {confirmLabel}
          </button>
