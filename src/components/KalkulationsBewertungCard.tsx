    ...
    <div className={`bg-white rounded-2xl border ${cfg.border} overflow-hidden`}>
      ...
      >
        ...
        <span className="flex-1 text-left">
          <span className="font-black text-[#2C2C2C] text-sm">KI-Kalkulation</span>
          <span className={`ml-2 text-[11px] font-black px-2 py-0.5 rounded-full border ${cfg.badge}`}>
            {cfg.label}
          </span>
        </span>
        <ChevronRight size={16} className={`text-[#2C2C2C]/30 transition-transform ${open ? 'rotate-90' : ''}`} />
      </button>

      {open && (
        <div className="px-4 pb-4 flex flex-col gap-3 border-t border-[#2C2C2C]/5">
          {/* Vertrauensbalken */}
                ...
                className={`h-1.5 flex-1 rounded-full ${
                  ...
                  (bewertung.vertrauensstufe === 'hoch' ||
                   (bewertung.vertrauensstufe === 'mittel' && s === 'gering'))
                    ? cfg.bar + ' opacity-30' : 'bg-[#2C2C2C]/10'
                }`}
              />
            ))}
          </div>
          ...
          {/* Bewertungstext */}
          <p className="text-xs text-[#2C2C2C]/60 font-semibold leading-relaxed">
          ...
          {bewertung.erkannte_angaben.length > 0 && (
            <div>
              <div className="text-[11px] font-black text-[#2C2C2C]/40 uppercase tracking-wide mb-1.5">
                Erkannte Angaben
              </div>
