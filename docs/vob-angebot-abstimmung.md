...
| VOB-002 | **Drei verschiedene Verschnittsätze** im Code — 5 %, 10 %, 12 % | Head of Product Engineering | ✅ **erledigt (04.09.):** eine Quelle je Gewerk. Die Annahme im Angebot wird jetzt aus derselben Funktion abgeleitet, mit der die Menge gerechnet wird (`verschnittFuerVerlegung`), Fliesen über benannte Konstanten; die tote 10-%-Rückrechnung in `aufnahme-hinweise.ts` liest den Satz aus dem Titel; der GPT-Prompt in `angebot-verfeinern` nennt dieselben Werte. Tests prüfen die Eigenschaft, nicht die Zahl |
...
| VOB-004 | Übermessungshinweis erreicht das Kunden-PDF nicht (= G5 aus CoS-L-001) | Head of Product Engineering + Product Designer | ✅ **erledigt (01.09., Status hier war veraltet):** `pdf.tsx` setzt je Position eine Fußnotenmarke und druckt `UEBERMESSUNG_ERKLAERUNG` einmal unter der Positionsliste. Tests: `uebermessung-pdf`, `pdf-uebermessung-render`. Damit auch **LR-01 🔴 geschlossen** |
...
| VOB-010 | Zuschlags-Einheiten: Prozent im Titel, Euro im Preis (14 Einträge) | Head of Product Engineering | ✅ **erledigt (Katalog 01.09., Geldweg 04.09. über CoS-043):** alle 14 auf `unit: %` umgestellt, in der echten Datenbank nachgeprüft (keine Migrationslücke), `katalog-hygiene` sichert gegen Rückfall. Beim Testen des Geldwegs zusätzlich gefunden und behoben: sieben wortgleiche Titel unterschieden sich nur im Prozentsatz — der Elektro-Zuschlag bekam 25 % statt 50 %. Damit **LR-09 🟡 schließbar**. Zwei kleine Punkte zurück an CoS/Sandy (Ausreißer Sondermaße, Beschreibungstext dieses Gewerks) |
...
| VOB-013 | **Leibungsfläche wird rundherum statt dreiseitig gerechnet, Fensterbank doppelt** — Fund des Prüfmeisters | Head of Product Engineering | ✅ **erledigt (03.09., Status hier war veraltet):** dreiseitig (`br + 2×hoe`), Fensterbank nur noch in ihrer eigenen Position. Test: `vob013-leibungen`. Damit auch **LR-13 🟠 geschlossen** |
