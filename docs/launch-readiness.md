...
| # | Punkt | Gate | Status |
|---|---|---|---|
| 9.1 | Landingpage erklärt klar, was das Tool tut und für wen | G1 | 🔴 0 % — **17.09.: Es gibt zwei Seiten, und das ist der Kern des Punktes.** LIVE unter `sofortangebot.app` steht weiterhin eine reine Warteliste (Ueberschrift, ein Satz, E-Mail-Feld) — sie erklaert weder was das Produkt tut noch fuer wen, und erfuellt den Punkt nicht. Der NEUE Entwurf liegt im Hauptprojekt an der Wurzel hinter dem Schalter `NEXT_PUBLIC_COMING_SOON` und tut beides, ist aber **nicht veroeffentlicht und derzeit nirgends aufrufbar** (17.09., 17:42 UTC gemessen: ein Team, ein Vercel-Projekt `sofortangebot`, und die frueher hier genannte Entwurfs-Adresse antwortet mit 302 auf `vercel.com/login` — der Satz von der eigenen Adresse war ein Fehler des Chief of Staff, zurueckgezogen; CoS-P-032 legt eine Vorschau an) und enthaelt vier offene Stopper (MwSt.-Angabe trotz § 19, Knappheits-Zeile ohne Kunden, Beispiel-Behauptung, vier widerspruechliche Gratis-Versprechen). Bleibt auf 0: eine Seite, die nicht live ist, zaehlt nicht, und eine, die falsche Zusagen macht, erst recht nicht. Vorheriger Stand: ⚪ offen — nicht erhoben |
