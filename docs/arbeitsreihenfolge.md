...
| **Prüfmeister** | **Platz 1 ist zu: Engineerings zwei Fragen sind beantwortet** (Commit `78602d0`). (1) Die Soll-Tabelle zu Fall 7 ist **nachgezogen** — die zwei Grundierungen raus, Netto 1.407,30 € -> **1.198,05 €**, der Warnkasten fragt jetzt nach ihrem Wiederauftauchen. (2) Der Nebenbefund mit den 6,00 €/m² ist **geschlossen**: gegen `DEFAULT_PRICES` sind es 4,50 €, die 6,00 € stehen als zweite Zeile derselben Kategorie (`Grundieren (Haftgrund / Sperrgrund)`) — **Katalogfrage, kein Treffer-Fehler, kein Bauauftrag**. Danach **Themenspeicher Punkt 11 gemessen** (PM-134 bis PM-137, 14 grün · 3 Sperrklinken): die Verneinungsmaschine hat **drei** Grenzen (Satz · Teilsatz · ganzer Text), und aus den Nähten fallen 356,25 € stumm weg (PM-134) bzw. 465,90 € zu viel aufs Blatt (PM-135). **Punkt 20 im selben Lauf als PM-138 gemessen — Fallbasis 138.** Nächstes: Punkt 12 (dieselbe Klasse an der Menge) und Punkt 21 (ob PM-134 bis PM-136 auch in `sockelleisten-ausschluss.ts` und `raum-ausschluss.ts` sitzen). **PD-024** liegt beim Designer, kein Bauauftrag | niemanden |
