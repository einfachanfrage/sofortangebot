...
**Stand: 17.09.2026, 10:25 UTC · Chief of Staff**
*(ersetzt die Fassung von 10:10 UTC — diese Datei wird immer ersetzt, nie ergänzt.)*
...
**Sandy hat dreimal geliefert: CoS-P-013 ist zu**, die **IONOS-Weiterleitung
`rechnung@` steht**, und der **Zustelltest ist bestanden** — `.xml`-Anhänge
kommen unverändert durch. Drei Punkte weg von ihrer Liste, keiner neu.
...
| Sandy | **Zustelltest E-Rechnung bestanden** — Mail aus ihrem Postfach an `rechnung@` kam in `hallo@` an, **mit allen drei Anhängen** | ✅ **Kanal belegt, nicht nur eingerichtet**, an Finance gemeldet |
...
* **Das Posteingangs-Bildschirmfoto selbst angesehen:** Mail von
  `einfachanfrage@outlook.com` an `rechnung@`, in `hallo@` angekommen, mit
  `zugferd-cii.xml`, `zugferd-rechnung.pdf` und `xrechnung-ubl.xml` — alle
  drei Anhänge sind durch die Weiterleitung gekommen.
...
* **Ob die Anhänge inhaltlich unversehrt ankommen**, habe ich nicht geprüft —
  ich sehe drei Dateinamen im Posteingang, nicht ihren Inhalt. Für die
  Verfahrensdokumentation an Finance gemeldet.
...
| 5 | 🔵 **Buchhaltungs-Testlauf** (Lexware Office) · **E-Rechnungs-Viewer** (Quba, 0 €) | nicht eilig |
| 6 | Vercel-Benachrichtigung · Versicherung · Stripe · Gewerbeanmeldung KW 41 (CoS-041) | unverändert, in diesem Lauf nicht geprüft |

**Erledigt und weg von ihrer Liste:** **CoS-P-013** (beantwortet),
die **IONOS-Weiterleitung `rechnung@`** (selbst eingerichtet) und der
**Zustelltest** (bestanden). Drei Punkte an einem Vormittag, nichts Neues
dafür hinzugekommen. **Vier offene Punkte statt sechs.**
...
| **Finance** | **Neu und sein Zug: der Empfangskanal `rechnung@` ist vollständig UND zugestellt-belegt**, `.xml` kommt durch. Ob Gate-1-Punkt 4.7 dadurch über die 40/100 steigt, entscheidet er, nicht ich. Sonst: F-007 gestellt, Belegablage steht, Reverse-Charge geklärt | Sandys Satz zur Sicherung · Sandys A/B zu § 19 |
