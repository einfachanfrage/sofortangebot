// Validiert alle 637 Testfälle aus testfaelle-v2.json
// - Bestehende Kategorien: Engine-Validierung (Menge muss stimmen)
// - Neue Erweiterungs-Kategorien: Struktur-Validierung (Felder vorhanden)
import { readFileSync } from 'fs'

const round2 = n => Math.round(n * 100) / 100
const TOLERANZ = 0.12

// Kategorien die die Engine kennt
const ENGINE_KATEGORIEN = new Set([
  'einzelraum_komplett','nur_waende','nur_decke','multi_raum','keller_garage',
  'standardhoehe_annahme','bad_komplett_nassbereich','nur_boden','altfliesen',
  'multi_bereich','terrasse','eingang','waschkueche','pool','dusche','wc',
  'kueche','bad_gross','bad_klein','flur','standard_verlegen','mit_altbelag',
  'diagonal','mit_sockelleisten','untergrundausgleich','linoleum',
])

// Neue Kategorien → nur Struktur prüfen
const STRUKTUR_KATEGORIEN = new Set([
  'bodengleiche_dusche','silikonfuge','fliesengroesse','naturstein','terrasse_aussen',
  'fliesenspiegel_kueche','parkett_schleifen','trittschalldaemmung','teppichboden',
  'fussbodenheizung','tueren_unterkuerzen','uebergangsprofile',
  'tapete_abloesen','schimmel','spachteln_grundieren','tueren_heizkoerper',
  'treppenhaus','fassade',
])
...
// ── Validierung ───────────────────────────────────────────────────
const { testfaelle } = JSON.parse(readFileSync('/sessions/sharp-clever-cerf/mnt/sofortangebot/tests/testfaelle-v2.json','utf8'))

let engineOk=0, engineFail=0
let strukturOk=0, strukturFail=0
let fehlerLog=[]

for (const tc of testfaelle) {
  if (tc.engine_test_skip) continue

  const istEngineKategorie = ENGINE_KATEGORIEN.has(tc.kategorie)
  const istStrukturKategorie = STRUKTUR_KATEGORIEN.has(tc.kategorie)

  if (istEngineKategorie) {
    // Strenge Engine-Validierung
  ...
  } else if (istStrukturKategorie) {
    // Struktur-Validierung: transkript, positionen_erwartet mit menge+einheit
    let fehler = false
    if (!tc.transkript || tc.transkript.length < 10) {
      fehlerLog.push(`[${tc.id}] KEIN TRANSKRIPT`); fehler = true
    }
    if (!tc.positionen_erwartet || tc.positionen_erwartet.length === 0) {
      fehlerLog.push(`[${tc.id}] KEINE POSITIONEN`); fehler = true
    }
    for (const p of (tc.positionen_erwartet ?? [])) {
      if (!p.beschreibung) { fehlerLog.push(`[${tc.id}] POSITION OHNE BESCHREIBUNG`); fehler = true }
      if (p.menge === undefined || p.menge === null) { fehlerLog.push(`[${tc.id}] POSITION OHNE MENGE: ${p.beschreibung}`); fehler = true }
      if (!p.einheit) { fehlerLog.push(`[${tc.id}] POSITION OHNE EINHEIT: ${p.beschreibung}`); fehler = true }
    }
    if (fehler) strukturFail++
    else strukturOk++
  }
}

const gesamt = engineOk + engineFail + strukturOk + strukturFail
console.log('\n══════════════════════════════════════════')
console.log('  VALIDIERUNG — testfaelle-v2.json')
console.log('══════════════════════════════════════════')
console.log(`\n📐 ENGINE-TESTS (Rechenlogik geprüft):`)
console.log(`   ✅ ${engineOk} bestanden  ❌ ${engineFail} fehlgeschlagen`)
console.log(`\n📋 STRUKTUR-TESTS (neue Kategorien):`)
console.log(`   ✅ ${strukturOk} Fälle OK  ❌ ${strukturFail} Fälle fehlerhaft`)
console.log(`\n📊 Gesamt: ${testfaelle.length} Testfälle, ${gesamt} Assertions`)
...
process.exit((engineFail + strukturFail) > 0 ? 1 : 0)
